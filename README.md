...
- **The Void · 极致输入区**
  ...
  - **下一步候选**：基于输入内容生成 3 个“下一步任务候选”并放入收集箱（可编辑后入历；不依赖 AI）
- **灵感星空 / 墓地**
...
### 3) AI 功能偶尔失败 / 回退本地
AI 相关功能（例如“灵感拆解”“乱炖”“从星入历”等）会通过 Netlify Function 代理调用 DeepSeek。网络波动、限流或模型输出不规整都可能导致失败；本项目会自动回退到本地模拟/模板以保证基础流程可用。
