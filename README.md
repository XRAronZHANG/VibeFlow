...
- **The Void · 极致输入区**
  ...
  - **给我一条路**：生成「现在的 3 分钟微行动 + 可入历的任务路线（自动识别期限，默认 7 天，支持 2/7/14 天切换）」，可一键入历（AI / 失败回退本地）
- **灵感星空 / 墓地**
...
### 3) “给我一条路”偶尔变得模板化
这是因为 AI 偶尔会输出不完全符合要求的严格 JSON（例如多输出解释文字、字段缺失、日期格式不标准）。本项目已加入 **两层稳定策略**：
1) 容错解析与自动补齐：尽量保留 AI 内容并修复缺失项（日期/字段/条数）。
2) JSON 修复重试：当输出不规整或内容覆盖率太低时，会自动再请求一次“修复为严格 JSON”。
只有在网络/代理失败或两次都无法得到可解析结构时，才会完全回退到本地模板。
